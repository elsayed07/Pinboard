...
def register(request, payload: RegisterIn):
    try:
        ...
        return Status(201, {"access": tokens.access, "refresh": tokens.refresh})
...
def login(request, payload: LoginIn):
    try:
        ...
        tokens = AuthService.get_tokens_for_user(user)
        return Status(200, {"access": tokens.access, "refresh": tokens.refresh})
    except ApplicationError as exc:
        return Status(exc.code, {"detail": exc.message})
...
def follow(request, target_id: str):
    try:
        ...
        return Status(201, {"status": "following"})
    except ApplicationError as exc:
        return Status(exc.code, {"detail": exc.message})
